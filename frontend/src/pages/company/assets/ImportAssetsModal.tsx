...
import { useInspectAssetImport, useImportAssets } from '@/api/hooks/assets'
import type { AssetSheetInfo, CustomFieldResponse, ImportResult } from '@/api/types'
...
export function ImportAssetsModal({ open, onClose, activeFields }: ImportAssetsModalProps) {
  const toast = useToast()
  const inspect = useInspectAssetImport()
  const runImport = useImportAssets()

  const [file, setFile] = useState<File | null>(null)
  const [sheet, setSheet] = useState<AssetSheetInfo | null>(null)
  ...
  const canImport = !!file && !!mappings.asset_name && !!mappings.category
  ...
  const doImport = async () => {
    ...
    try {
      ...
      setResult(res)
      if (res.imported > 0) toast.success(`Imported ${res.imported} asset(s)`)
    } catch (e) {
  ...
  return (
    <Modal open={open} onClose={close} title="Import assets" footer={footer} size="lg">
      {result != null ? (
        ...
        <div className="flex flex-col gap-4">
          ...
          {sheet && (
            <div className="flex flex-col gap-3">
              ...
              {!canImport && (
                <p className="text-xs text-status-action">Map at least Asset name and Category to import.</p>
              )}
            </div>
          )}
        </div>
